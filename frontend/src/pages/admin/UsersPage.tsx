...
export const UsersPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (currentUser) {
        // Edit flow
        try {
          await apiV1Client.put(`/users/${currentUser.id}`, formData);
        } catch {
          // Local fallback
        }
        setUsers((prev) =>
          prev.map((u) => (u.id === currentUser.id ? ({ ...u, ...formData } as UserItem) : u))
        );
        ...
        try {
          await apiV1Client.post('/users', formData);
        } catch {
          // Local fallback
        }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      triggerAlert('success', `Akun pengguna "${currentUser.name}" berhasil dihapus.`);
    ...
    <div id="users-page" className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">Kelola akun pengguna administrator dan staf editor yang memiliki otorisasi akses CMS</p>
        </div>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="p-12 text-center text-zinc-500">
                    Tidak ada akun pengguna yang terdaftar atau cocok dengan pencarian.
                  </td>
                </tr>
              ) : (
                filteredUsers.map((u) => (
      ...
      {/* Create/Edit Modal */}
      {isModalOpen && (
        <div id="user-modal" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl animate-fade-in">
            <div className="flex items-center justify-between p-6 border-b border-zinc-800">
              <h2 className="text-lg font-bold text-white">
                {currentUser ? 'Sunting Otoritas Akun' : 'Add New User Account'}
              </h2>
              <button
                id="btn-user-close"
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Contoh: Editor Kreasi"
                  required
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Alamat Email</label>
                <input
                  type="email"
                  name="email"
                  value={formData.email || ''}
                  onChange={handleInputChange}
                  placeholder="Contoh: editor@kreasi.com"
                  required
                  className="w-full px-4 py-2.5 bg-zinc-950 border border-zinc-800 rounded-xl text-white placeholder-zinc-600 focus:outline-none focus:border-amber-500 text-sm"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Akses Otoritas (Role)</label>
                <select
                  name="role"
                ...
                >
                  <option value="editor">Editor (Manajemen Konten Sahaja)</option>
                  <option value="admin">Administrator (Akses Penuh Konsol)</option>
                </select>
              </div>
      ...
      {isDeleteOpen && (
        <div id="user-delete-modal" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-sm">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-zinc-400 text-sm mt-1">
                  Apakah Anda yakin ingin menghapus akun pengguna <strong className="text-white">"{currentUser?.name}"</strong>? Tindakan ini tidak dapat dibatalkan.
                </p>
              </div>
            </div>
